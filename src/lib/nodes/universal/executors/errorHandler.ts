...
export async function executeWithErrorHandling<T>(
  ...
  stepInfo?: { type: string; number?: number; field?: string }
): Promise<T> {
  const {
    retry = 0,
    retryDelay = 1000,
  ...
  while (attempt <= retry) {
    try {
      ...
      console.warn(
        `[ErrorHandler] ${stepInfo?.type || 'Step'} failed (attempt ${attempt}/${retry + 1}):`,
        lastError.message
      );
  ...
  console.error(
    `[ErrorHandler] ${stepInfo?.type || 'Step'} failed after ${attempt} attempts. Strategy: ${onError}`
  );
...
export function enhanceError(
  ...
  if (context.stepType) contextParts.push(`step type: ${context.stepType}`);
  if (context.stepNumber) contextParts.push(`step #${context.stepNumber}`);
  if (context.iteration) contextParts.push(`iteration ${context.iteration}`);
  if (context.outputField) contextParts.push(`output field: ${context.outputField}`);
