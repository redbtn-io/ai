...
/**
 * Output structure for connection step
 */
interface ConnectionOutput {
  /** Pre-built authentication headers ready for fetch/axios */
  headers: Record<string, string>;
  /** Account info (if include = 'account' or 'full') */
  accountInfo?: {
    email?: string;
    name?: string;
    avatarUrl?: string;
 ...
 */
async function executeConnectionInternal(
  config: ConnectionStepConfig,
  state: any
): Promise<Partial<any>> {
  ...
  const connectionManager = state.connectionManager as ConnectionManager | undefined;
